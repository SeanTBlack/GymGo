...
0 - Black pieces "BLACK"
1 - White pieces "WHITE"
2 - Turn (0 - black, 1 - white) "TURN_CHNL"
3 - Invalid moves (including ko-protection) "INVD_CHNL"
4 - Previous move was a pass "PASS_CHNL"
5 - Game over "DONE_CHNL"
"""
...
def game_ended(state):
    ...
    """
    m, n = state.shape[1:]
    #print("game_ended function")
    #print(state[3])
    #print(state.shape)
    #print(state[govars.DONE_CHNL])
    #print(state[govars.INVD_CHNL])
    #print(np.count_nonzero(state[govars.DONE_CHNL] == 1))

    if state[govars.DONE_CHNL].any() == 1:
        #print("done_chnl=1")
        return 1
    elif np.count_nonzero(state[govars.INVD_CHNL] == 1) == m * n:
        #print("All states invalid")
        return 1
    else:
        #print("Else")
        return 0
    ...
    #print(int(np.count_nonzero(state[govars.INVD_CHNL] == 1) == m * n))
    #return int(np.count_nonzero(state[govars.DONE_CHNL] == 1) == m * n) # Nothing is filling in the govars.DONE_CHNL. Not sure why
    #return int(np.count_nonzero(state[govars.INVD_CHNL] == 1) == m * n)
